...
namespace Base.API.Controllers.v1
{
    ...
    public class FilmeXDiretorController : BaseController
    {
        ...
        [HttpPost]
        public async Task<IActionResult> Post([FromBody] FilmeXDiretorInsertDTO filmeXDiretor)
        {
            try
            {               
                var retorno = (Retorno)await _rep.Cadastrar(filmeXDiretor);
        ...
        [HttpPut("{id}")]
        public async Task<IActionResult> Put([FromBody] FilmeXDiretorUpdateDTO filmeXDiretor)
        {
            try
            {
                var retorno = (Retorno)await _rep.Atualizar(filmeXDiretor);
                if (retorno.Sucesso == false)
                    return BadRequest(retorno);
        ...
        public async Task<IActionResult> Delete([FromBody] FilmeXDiretorDTO filmeXDiretor)
        {
            try
            {
                var retorno = (Retorno)await _rep.Excluir(filmeXDiretor);
                if (retorno.Sucesso == false)
                    return BadRequest(retorno);
