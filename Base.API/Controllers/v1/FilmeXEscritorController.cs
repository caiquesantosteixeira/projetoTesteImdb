...
namespace Base.API.Controllers.v1
{
    ...
    public class FilmeXEscritorController : BaseController
    {
        ...
        [HttpPost]
        public async Task<IActionResult> Post([FromBody] FilmeXEscritorDTO filmeXEscritor)
        {
            try
            {               
                var retorno = (Retorno)await _rep.Persistir(filmeXEscritor, ELogin.CADASTRAR);
        ...
        [HttpPut("{id}")]
        public async Task<IActionResult> Put([FromBody] FilmeXEscritorDTO filmeXEscritor)
        {
            try
            {
                var retorno = (Retorno)await _rep.Persistir(filmeXEscritor, ELogin.ATUALIZAR);
                if (retorno.Sucesso == false)
                    return BadRequest(retorno);
