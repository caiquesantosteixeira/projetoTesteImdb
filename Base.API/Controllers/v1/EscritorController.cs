...
namespace Base.API.Controllers.v1
{
    ...
    public class EscritorController : BaseController
    {
        ...
        [HttpPost]
        public async Task<IActionResult> Post([FromBody] EscritorInsertDTO escritor )
        {
            try
            {               
                var retorno = (Retorno)await _rep.Cadastrar(escritor);
                if (retorno.Sucesso == false)
                    return BadRequest(retorno);
        ...
        [HttpPut("{id}")]
        public async Task<IActionResult> Put([FromBody] EscritorUpdateDTO escritor)
        {
            try
            {
                var retorno = (Retorno)await _rep.Atualizar(escritor);
                if (retorno.Sucesso == false)
                    return BadRequest(retorno);
        ...
        public async Task<IActionResult> Delete([FromBody] EscritorDTO escritor)
        {
            try
            {
                var retorno = (Retorno)await _rep.Excluir(escritor);
                if (retorno.Sucesso == false)
                    return BadRequest(retorno);
